...
export class Bridge {
  ...
  private async handleAlert(alert: AlertEvent): Promise<void> {
    ...
    //    structurally similar to the EA1-019/022 prior anomalies so the
  ...
  private async promoteToAnomaly(
    ...
    // Author a description that mirrors the EA-1 019/022 story line so
    ...
    const morphologyClause =
      alert.sensor_kind === 'vibration'
        ? 'Vibration excursion morphology consistent with the EA1-019 and EA1-022 bearing-race micro-pitting pattern. Bearing race batch under traceability pull — Acme Bearings AB-7821 family.'
        : alert.sensor_kind === 'pressure'
          ? 'Pressure drift profile matches the LH-0 unit 031 pre-burner combustion-stability prior anomaly. Pre-burner injector batch under review.'
...
function inferProgramFromUnit(unit_id: string): string {
  if (unit_id.startsWith('EA1-')) return 'EA-1';
  if (unit_id.startsWith('LH0-')) return 'LH-0';
  if (unit_id.startsWith('NG-')) return 'NG';
  if (unit_id.startsWith('NS-')) return 'NS';
  if (unit_id.startsWith('LM-1-')) return 'LM-1';
  return 'EA-1';
}
