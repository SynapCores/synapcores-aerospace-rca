...
2. `pnpm seed-demo -- --bulk --hold-today` already executed (so the
   ...
   minus the EA-1 unit 027 anomaly which Act 1 ingests live).
3. Browser at `http://localhost:3005/demo`, viewport 1440x900,
   sidebar visible.

---

## Act 1 · 06:14:23 UTC — anomaly detected on EA-1 hot-fire stand 4 (~12s)
...
program badges. The LH-0 unit 031 row glows red — "different program,
...
via `GET /api/v1/anomalies/ANM-2026-EA1-027/similar?k=6`.

**Caption.** *"4 prior matches. One on LH-0 — heritage engine — that a
SharePoint search would never have linked."*
...
programs (purple) — EA-1, LH-0, NG. Edges OCCURRED_ON, SUPPLIED_BY,
USED_IN. A side-panel renders the supplier summary:
*"Supplier: Acme Precision Bearings · Same bearing race ships into EA-1,
LH-0, NG."*

**Queries fired** via `GET /api/v1/anomalies/ANM-2026-EA1-027/graph`,
which issues these MATCHes against `/v1/query/execute`:

```cypher
MATCH (a:Anomaly)-[:OCCURRED_ON]->(p:Part)-[:SUPPLIED_BY]->(s:Supplier)
  WHERE a.id = "ANM-2026-EA1-027" RETURN p, s
...
j.warren@example.com has left the company (HR registry)"* line in
destructive-red. A small badge bottom: **"OIG AR-26-004 — nearly half
of PDR RFAs remain open >1 year"**.

**Queries fired** (server-side via `POST /api/v1/anomalies/ANM-2026-EA1-027/agent`
...
**Queries fired** via `GET /api/v1/audit?target=ANM-2026-EA1-027`:
...
**On-screen.** The page header reads "DCU — Live Telemetry · EA-1
...
## Act 2 · t≈12s — vibration spike, EA-1 unit 027 LOX turbopump

**On-screen.** Sparkline 1 (EA1-027-TP-VIB-X-014) goes vertical. The
...
button. The promoted anomaly id starts with `ANM-LIVE-EA1-027-...`.

**Caption.** *"The DCU's rolling z-score caught a 12σ excursion. The
bridge promoted it into the same anomalies table the past 45 EA-1
investigations live in. One click and we're in the U1 surface."*

## Act 3 · t≈31s — pressure drift, HL1-2 pre-burner

**On-screen.** Sparkline 2 (HL1-2-PB-PRES-002) starts ramping. The
...
## Act 4 · t≈53s — single-sample temp excursion, LH-0 unit 031 *(detector debounces — NO alert)*

**On-screen.** Sparkline 3 (LH0-031-TP-TEMP-007) shows one sample at
...
## Act 5 · t≈71s — second EA1-027 sensor fires; cluster pattern

**On-screen.** Sparkline 4 (EA1-027-CC-VIB-Y-022) spikes. New alert
appears with a red **"cluster: 2 sensors on EA1-027 this run"** label.
The promoted anomaly's description explicitly references the prior
EA1-027 alert.

**Caption.** *"This is the second sensor on unit EA1-027 to fire in 60
...
Act 2 alert. Browser navigates to `/anomalies/ANM-LIVE-EA1-027-XXXXXX`
— the existing U1 detail page. The "Similar past anomalies (vector
recall)" card populates with the top-5 cosine matches:

- ANM-2024-LH0-007 — LH-0 turbopump bearing race carbon residue
- ANM-2024-EA1-022 — Repeat LOX bearing race carbon deposit, EA-1 022
- ANM-2026-HL1-010 — Methane-side turbopump bearing race vibration
- ANM-2026-EA1-027 — EA-1 unit 027 LOX bearing race carbon deposit
- ANM-2023-EA1-019 — LOX turbopump bearing race micro-pitting
...
|    12 | Act 2 alert — EA1-027-TP-VIB-X-014                   |
|    31 | Act 3 alert — HL1-2-PB-PRES-002                       |
|    53 | Act 4 — temp single-sample event (NO alert fires)    |
|    71 | Act 5 alert — EA1-027-CC-VIB-Y-022, cluster label    |
