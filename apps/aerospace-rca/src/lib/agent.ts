...
export async function runReliabilityEngineer(
  ...
  const recommended_action = unprotected.length
    ? `Open propagation review for ${unprotected
        .map((u) => u.program)
        .join(', ')}; mirror the verified EA-1 corrective actions onto those programs' supplier-acceptance flows.`
...
export async function runSafetyOfficer(
  ...
  const recommended_action = stale.length
    ? 'Notify LM-1 / NG program management; recommend supplier-hold on Acme Bearings batches pre-24Q2 + immediate ownership reassignment for RFAs flagged as departed-employee-owned.'
