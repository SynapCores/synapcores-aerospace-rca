...
export async function POST() {
  try {
    const id = await ingestAnomaly({
      ...
      severity: 'major',
      status: 'open',
      title: 'EA-1 unit 027 LOX bearing race carbon deposit, vibration excursion',
      description:
        'Carbon deposits exceed spec on LOX-side turbopump bearing race during 14-second hot-fire test on 2026-06-12T06:14:00Z. Vibration signature shifted by 3.2 sigma at T+0.7s on the high-pressure oxidizer shaft. Bearing race showed micro-pitting consistent with debris from upstream contamination, morphology near-identical to the EA1-019 and EA1-022 incidents. Bearing race batch under traceability pull — Acme Bearings AB-7821 family. Stand 4 instrumentation nominal; LOX prevalve and feedline witness samples taken.',
      reporter: 'K. Suresh',
      test_stand: 'Hot-fire Stand 4',
      source_doc: 'PFR-2026-EA1-027.pdf',
    });
