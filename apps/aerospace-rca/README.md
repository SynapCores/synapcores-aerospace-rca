...
**Aerospace anomaly Root Cause Analysis.** Built for the customer outreach
...
EA-1, LH-0, NS, NG, and the Lunar Module Mk2 (LM-1), narrated as a single
...
#    --hold-today reserves the EA-1 unit 027 anomaly for live ingest at
...
- `/anomalies` lists rows filterable by program + severity; clicking
  ...
  Officer agent.
- `/rfas` shows the Federal Aerospace Regulator audit shape — open, aging, departed-owner
  highlighted.
...
|------:|----------------------------|------------------------------|
|    12 | EA1-027-TP-VIB-X-014       | debounced-z, vibration       |
|    31 | HL1-2-PB-PRES-002           | step-detector OR z-score     |
|    53 | LH0-031-TP-TEMP-007        | **suppressed** (debounce=4)  |
|    71 | EA1-027-CC-VIB-Y-022       | debounced-z, vibration       |

When alerts 1 and 4 fire, the bridge promotes them into the existing
`anomalies` table — same `unit_id=EA1-027`, similar morphology, with
...
where vector recall surfaces EA-1 019, EA-1 022, LH-0 007, HL1-010
...
Alert 4 fires on a SECOND sensor of unit EA1-027 — the cluster
