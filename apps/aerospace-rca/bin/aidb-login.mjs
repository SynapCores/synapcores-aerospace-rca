 ...
 * POST /v1/auth/login, and print ONLY the access_token to stdout so an
 * entrypoint can capture it:
 *
 *   export SYNAPCORES_ADMIN_API_KEY="$(node bin/aidb-login.mjs)"
 *
 * The engine mints the token (signed with AIDB_JWT_SECRET); we never sign
 * anything client-side. Retries until the gateway is up AND the first-boot
 * admin user exists (schema init can lag container start). All diagnostics
 * go to stderr so stdout stays clean for the token.
 */
